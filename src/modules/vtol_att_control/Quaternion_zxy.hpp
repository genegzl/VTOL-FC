...
//TODO: Only modify the euler to quaternion function
...
/**
 * Quaternion_zxy class
 *
 * The rotation between two coordinate frames is
 * described by this class.
 */
template<typename Type>
class Quaternion_zxy : public Vector<Type, 4>
{
public:
     ...
     * @param data_ array
     */
    Quaternion_zxy(const Type data_[4]) :
        Vector<Type, 4>(data_)
    {
    }
     ...
     */
    Quaternion_zxy(const Matrix41 &other) :
        Vector<Type, 4>(other)
    {
    }
     ...
     */
    Quaternion_zxy(const Dcm<Type> &R)
    {
        Quaternion_zxy &q = *this;
        Type t = R.trace();
        if (t > Type(0)) {
     ...
     */
    Quaternion_zxy(const Euler<Type> &euler)
    {
        Quaternion_zxy &q = *this;
     ...
     */
    Quaternion_zxy(const AxisAngle<Type> &aa)
    {
        Quaternion_zxy &q = *this;
        Type angle = aa.norm();
    ...
    /**
     * Quaternion_zxy from two vectors
     ...
     */
    Quaternion_zxy(const Vector3<Type> &src, const Vector3<Type> &dst, const Type eps = Type(1e-5))
    {
        Quaternion_zxy &q = *this;
     ...
     */
    Quaternion_zxy(Type a, Type b, Type c, Type d)
    {
        Quaternion_zxy &q = *this;
        q(0) = a;
        q(1) = b;
    ...
    /**
     * Quaternion_zxy multiplication operator
     *
     * @param q Quaternion_zxy to multiply with
     * @return product
     */
    Quaternion_zxy operator*(const Quaternion_zxy &q) const
    {
        const Quaternion_zxy &p = *this;
        Quaternion_zxy r;
     ...
     */
    void operator*=(const Quaternion_zxy &other)
    {
        Quaternion_zxy &self = *this;
        self = self * other;
    }
     ...
     * @return product
     */
    Quaternion_zxy operator*(Type scalar) const
    {
        const Quaternion_zxy &q = *this;
        return scalar * q;
    }
    ...
    {
        const Quaternion_zxy &q = *this;
        Quaternion_zxy<Type> v(0, w(0, 0), w(1, 0), w(2, 0));
        return q * v  * Type(0.5);
    }
    ...
    /**
     * Invert Quaternion_zxy in place
     */
    void invert()
    {
        *this = this->inversed();
    ...
    /**
     * Invert Quaternion_zxy
     *
     * @return inverted Quaternion_zxy
     */
    Quaternion_zxy inversed() const
    {
        const Quaternion_zxy &q = *this;
        Type normSq = q.dot(q);
        return Quaternion_zxy(
                   q(0)/normSq,
                   -q(1)/normSq,
                   -q(2)/normSq,
                   -q(3)/normSq);
    ...
    /**
     * Rotate Quaternion_zxy from rotation vector
     *
     * @param vec rotation vector
     */
    void rotate(const AxisAngle<Type> &vec)
    {
        Quaternion_zxy res(vec);
        (*this) = res * (*this);
    }

    /**
     * Rotates vector v_1 in frame 1 to vector v_2 in frame 2
     * using the rotation Quaternion_zxy q_21
    ...
    Vector3f conjugate(const Vector3f &vec) {
        Quaternion_zxy q = *this;
        Quaternion_zxy v(0, vec(0), vec(1), vec(2));
        Quaternion_zxy res = q*v*q.inversed();
    ...
    Vector3f conjugate_inversed(const Vector3f &vec) const
    {
        Quaternion_zxy q = *this;
        Quaternion_zxy v(0, vec(0), vec(1), vec(2));
        Quaternion_zxy res = q.inversed()*v*q;
        return Vector3f(res(1), res(2), res(3));
    }

    /**
     * Rotation Quaternion_zxy from vector
     *
     * The axis of rotation is given by vector direction and
     * the angle is given by the norm.
     *
     * @param vec rotation vector
     * @return Quaternion_zxy representing the rotation
     */
    ...
    /**
     * Rotation Quaternion_zxy from axis and angle
    ...
    /**
     * Rotation vector from Quaternion_zxy
     * XXX DEPRECATED, use AxisAngle class
     *
     * The axis of rotation is given by vector direction and
     * the angle is given by the norm.
    ...
    /**
     * Imaginary components of Quaternion_zxy
     */
    ...
    /**
     * Corresponding body z-axis to an attitude Quaternion_zxy /
     ...
     */
    Quaternion_zxy from_dcm(const Matrix<Type, 3, 3>& dcm) {
        return Quaternion_zxy(Dcmf(dcm));
    }
...
typedef Quaternion_zxy<float> Quatf_zxy;
typedef Quaternion_zxy<float> Quaternionf_zxy;
